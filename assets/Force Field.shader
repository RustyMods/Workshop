...
Shader "Ultimate 10+ Shaders/Force Field"
{
    Properties
    {
        ...
        _Power("Force Field Power", Range(0, 1)) = 1
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                fixed4 ffPixel = tex2D(_MainTex, input.uv) * _Color * pow(_FresnelPower, input.rim);
                ffPixel = lerp(0, ffPixel, input.rim);
                ffPixel = clamp(ffPixel, 0, _Color);
                
                fixed4 opaquePixel = tex2D(_MainTex, input.uv) * _Color;
                
                pixel = lerp(opaquePixel, ffPixel, _Power);
